...
Most scripting languages store variables as pointers to heap-allocated objects managed by a garbage collector. Vega stores variables as raw bytes inside a flat `[]byte` buffer managed by a free-list allocator. Variable names are erased at compile time — the runtime operates on slot IDs and byte offsets.
...
|---------|------|--------|-----|---|------|-------------|
| Variable storage | Byte array (global allocator) | Heap objects | Heap objects | Stack/heap | Stack/heap | Linear memory |
| Memory management | Explicit `free()` + function-scoped auto-free | GC (reference counting + cycle collector) | GC (incremental mark-and-sweep) | Manual `malloc`/`free` | Ownership + borrow checker | Host-managed |
...
### 1. Variables Are Byte Offsets, Not Objects
...
2. At runtime, the global allocator reserves 4 bytes at some offset.
...
### 2. Explicit Deallocation Without Ownership or Borrowing
...
### 3. Function Locals Are Automatically Freed

Unlike C's stack (which is reclaimed only when the function frame is popped) or C's heap (which requires explicit `free`), Vega function locals are freed on return by the runtime — without GC involvement.

On every function return, the runtime iterates the callee's slot table and calls `allocator.Free()` for each alive, non-alias slot. This is deterministic (no GC pause), immediate (bytes return to the pool right away), and automatic (the programmer doesn't need to free locals manually).
...
```
x = 42           # int at some offset
y = *short(0)    # view the first 2 bytes of the buffer as a short
...
- **Freed** explicitly by `free()`, automatically on function return, or at session end.
...
|--------|------|-------------|
| Memory region | Single global buffer (1 MiB default) | Module declares initial/max memory pages |
| Variable access | Slot ID → offset lookup → byte read | Local index → typed value (separate from linear memory) |
| Deallocation | `free()` or auto-free on function return | No built-in allocator; must implement in user code |
| Growth | Fixed at session start | `memory.grow` adds pages |
| Safety | Use-after-free detection, bounds-checked aliases | Bounds-checked memory access |
...
Vega's variables are similar — they get byte offsets determined by the allocator. But Vega adds:
- **Dynamic allocation** within the session (variables can be created at any point, not just at function entry).
...
- **Persistence** across calls (the global allocator survives `Run()` boundaries in REPL mode).
...
The byte-array model fits these constraints:
- A 1 MiB pool is ample for orchestration scripts while providing a hard upper bound on memory use.
- No GC means predictable performance (no pauses, no overhead).
- Explicit `free()` encourages scripts to release resources promptly.
- Function-local auto-free prevents resource leaks without programmer discipline.
